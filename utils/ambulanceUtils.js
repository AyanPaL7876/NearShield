...
export const fetchAmbulanceStations = async (
  latitude,
  longitude,
  searchRadius,
  useFallbackData,
  ...
  try {
    const url = `https://overpass-api.de/api/interpreter?data=[out:json];(node(around:${searchRadius},${latitude},${longitude})["amenity"="hospital"]["emergency"="yes"];node(around:${searchRadius},${latitude},${longitude})["healthcare"="hospital"];);out body;`;
    ...
    if (data?.elements?.length > 0) {
      const stationsWithDistance = data.elements.map(station => {
        ...
        return {
          ...
          name: tags.name || 'Ambulance Station',
          geometry: {
            location: {
              lat: station.lat,
              lng: station.lon
      ...
      // No stations found, return fallback data
